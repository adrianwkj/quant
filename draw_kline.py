...
if __name__ == '__main__':
    ...
    lines_data = pd.read_sql(
        "select trade_date, price from stroke where symbol = '%s' and circle = '%s'" % (symbol, circle), conn
    )

    lines_trade_date = lines_data['trade_date'].values
    lines_price = lines_data['price'].values
    # points = list(map(lambda x: list(zip(points_trade_date, x)), points_price))
    lines = list(zip(map(str, map(np.datetime_as_string, lines_trade_date)), lines_price))
    ...
    points_data = pd.read_sql(
        "select trade_time as trade_date, bid_ask from point where symbol = '%s' and circle = '%s'" % (symbol, circle), conn
    )
    points_data.set_index('trade_date', inplace=True)
    bid_ask = data.join(points_data, on='trade_date')
